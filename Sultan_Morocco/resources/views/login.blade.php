...
<body class="bg-custom-gray text-[#1B1B18] min-h-screen flex flex-col justify-center items-center relative overflow-hidden">
    ...
    <svg class="absolute bottom-10 right-[-5%] w-[500px] h-[400px] opacity-60 z-0 bg-decorative pointer-events-none animate-float-alt" viewBox="0 0 100 100" preserveAspectRatio="none">
        <!-- Simplified Kasbah/Castle silhouette -->
    ...
    <!-- Main Card -->
    <main class="relative z-10 w-full max-w-[420px] bg-white rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.06)] px-10 py-12 slide-up">
        <div class="text-center mb-10 slide-up delay-100">
            <h1 class="font-serif italic text-3xl text-green-dark mb-4 tracking-wide hover:scale-105 transition-transform duration-500 cursor-default">Sultan Morocco</h1>
            <h2 class="text-2xl font-bold mb-1">Welcome Back</h2>
            <p class="text-gray-500 text-sm">Login to explore Morocco</p>
        </div>

        <form action="{{ route('login') }}" method="POST">
            @csrf
            <div class="mb-5 slide-up delay-200">
                <label for="email" class="block text-[10px] uppercase tracking-widest text-gray-500 font-semibold mb-2 ml-4">Email Address</label>
                <div class="relative group">
                    <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full h-[52px] bg-input rounded-full px-5 text-sm outline-none transition-all duration-300 font-medium text-gray-700 hover:bg-gray-200/50 @error('email') border-red-500 ring-2 ring-red-500/50 @enderror" placeholder="curator@sultanmorocco.com" required>
                </div>
                @error('email')
            ...
            <div class="mb-8 slide-up delay-300">
                <div class="flex justify-between items-center mb-2 ml-4 mr-4">
                    <label for="password" class="block text-[10px] uppercase tracking-widest text-gray-500 font-semibold">Password</label>
                    <a href="#" class="text-[10px] text-green-dark font-bold hover:text-emerald-500 transition-colors">Forgot password?</a>
                </div>
                <div class="relative group">
                    <input type="password" id="password" name="password" class="w-full h-[52px] bg-input rounded-full px-5 pr-12 text-sm outline-none tracking-widest transition-all duration-300 font-bold text-gray-700 hover:bg-gray-200/50 @error('password') border-red-500 ring-2 ring-red-500/50 @enderror" placeholder="••••••••" required>
                    <button type="button" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors pointer-cursor hover:scale-110 active:scale-95 duration-200">
                        <!-- Eye Icon -->
